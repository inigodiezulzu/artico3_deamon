...
int artico3d_handle_request() {
    ...
    while (1) {
        ...
        *tdata = coordinator->request;
        ret = artico3_pool_submit_task(requests_pool, 0, _artico3d_handle_request, tdata);
        if(ret) {
...
int artico3d_kernel_execute(void *args) {
    ...
    tdata[0] = id;
    tdata[1] = nrounds;
    ret = artico3_pool_submit_task(kernels_pool, id, _artico3d_kernel_execute, tdata);
    if (ret == -1) {
